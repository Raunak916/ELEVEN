---
name: image-to-code
description: Elite website image-to-code skill. For visually important web tasks, it first generates design image references (via generate_image), deeply analyzes them (visual hierarchy, type scale, spacing, color tokens, layout), then implements the frontend code matching the visual reference with high fidelity.
---
...
IMPORTANT:
For visual website tasks, you must first generate the design image(s) yourself using the image generation tool.
...
When this skill is used in an environment that supports image generation plus implementation, default to an image-first workflow for visual website tasks.

Execution order:
1. Infer the section count & visual direction
2. Generate section reference images first (via generate_image)
3. Generate extra detail/extraction images where needed
4. If needed, regenerate unclear sections as fresh standalone images
5. Deeply inspect all generated images for visual tokens, layout, typography, and spacing
6. Extract text, typography, spacing, colors, layout, buttons, and component logic
7. Implement the website components to match the generated design as closely as possible
