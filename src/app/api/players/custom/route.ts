...
import { getPlayerDB } from '@/lib/player-db';
...
interface CustomPlayerInput {
  ...
  // photo will be handled as file upload
}
...
async function checkDuplicateCustomPlayer(db: any, name: string, team: string): Promise<any | null> {
  // Check for similar custom players (same name and club)
  const existing = db.prepare(`
    SELECT * FROM players
    WHERE source = 'custom'
    AND LOWER(name) = LOWER(?)
    AND LOWER(current_team) = LOWER(?)
  `).get(name.trim(), team.trim());
...
export async function POST(request: NextRequest) {
  try {
    ...
    // Check for duplicate custom player
    const duplicate = await checkDuplicateCustomPlayer(db, body.name.trim(), body.team.trim());
    if (duplicate) {
      return NextResponse.json(
        {
    ...
    // Use basePrice as marketValueEur if marketValueEur not provided
    const marketValueEur = body.marketValueEur || body.basePrice || 0;

    // Create the player object for database
    const player = {
      id: customId,
      externalIds: {},
      name: body.name.trim(),
    ...
    // Return the normalized player object matching the Player type
